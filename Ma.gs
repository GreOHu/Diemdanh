...
// STT | Họ Tên | Chức Vụ | Đơn Vị | Meeting ID | Thời Gian | Ngày | Device Token | Số Lần | Timestamp
const COL = {
...
function getOrCreateMeetingSheet(meetingID) {
  ...
  if (!sheet) {
    ...
    sheet.appendRow([
      ...
      'Thời Gian', 'Ngày', 'Device Token', 'Số Lần', 'Timestamp'
    ]);
...
function handleListMeetings(payload) {
  try {
    ...
    sheets.forEach(sheet => {
      ...
      if (name.indexOf('rollcall_') === 0) return; // sheet danh sách xác nhận lần 2, không phải cuộc họp
...
// thêm action polling riêng cho display.html. Danh sách AI ĐÃ XÁC NHẬN lần 2
// được ghi vào 1 sheet RIÊNG (rollcall_<meetingID>), tách bạch hoàn toàn khỏi
// sheet điểm danh gốc — không đụng tới COL/schema điểm danh lần 1.
//
// Cột trong sheet "rollcall_<meetingID>":
// STT | Token | Họ Tên | Device Token | Thời Gian | Ngày | Timestamp
const COL_RC = { STT: 1, TOKEN: 2, HOTEN: 3, DEVICE_TOKEN: 4, THOIGIAN: 5, NGAY: 6, TIMESTAMP: 7 };

function rollCallSheetName(meetingID) {
  return sanitizeSheetName('rollcall_' + meetingID);
}

function getOrCreateRollCallSheet(meetingID) {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const sheetName = rollCallSheetName(meetingID);
  let sheet = ss.getSheetByName(sheetName);
  if (!sheet) {
    sheet = ss.insertSheet(sheetName);
    sheet.appendRow(['STT', 'Token', 'Họ Tên', 'Device Token', 'Thời Gian', 'Ngày', 'Timestamp']);
    sheet.setFrozenRows(1);
  }
  return sheet;
}
...
function handleStartRollCall(payload) {
  try {
    ...
    // Đảm bảo sheet danh sách xác nhận lần 2 cho đợt này đã sẵn sàng.
    getOrCreateRollCallSheet(meetingID);
...
// Đại biểu quét QR lần 2, bấm "Xác nhận có mặt" trên trang riêng — ghi 1 dòng
// vào sheet rollcall_<meetingID>. Không cho xác nhận nếu token sai/đã hết hạn,
// và không cho xác nhận trùng trên cùng thiết bị (giống logic deviceToken ở
// điểm danh lần 1) để tránh 1 người bấm nhiều lần làm sai lệch số liệu.
function handleSubmitRollCall(payload) {
  ...
  try {
    ...
    // Xác định họ tên từ dữ liệu điểm danh lần 1 (qua deviceToken) — đại biểu
    // không cần nhập lại tên, chỉ cần bấm 1 nút xác nhận.
    let fullName = '';
    const attendanceSheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(sanitizeSheetName(meetingID));
    if (attendanceSheet && deviceToken) {
      const aData = attendanceSheet.getDataRange().getValues();
      for (let i = 1; i < aData.length; i++) {
        if (aData[i][COL.DEVICE_TOKEN - 1] === deviceToken) {
          fullName = aData[i][COL.HOTEN - 1];
          break;
        }
      }
    }
    ...
    const rcSheet = getOrCreateRollCallSheet(meetingID);
    const rcData = rcSheet.getDataRange().getValues();
    for (let i = 1; i < rcData.length; i++) {
      if (rcData[i][COL_RC.TOKEN - 1] === token && rcData[i][COL_RC.DEVICE_TOKEN - 1] === deviceToken) {
    ...
    const timeStr = Utilities.formatDate(timestamp, Session.getScriptTimeZone(), 'HH:mm:ss');
    const dateStr = Utilities.formatDate(timestamp, Session.getScriptTimeZone(), 'dd/MM/yyyy');
    const newRowNumber = rcSheet.getLastRow() + 1;
    rcSheet.appendRow([newRowNumber - 1, token, fullName, deviceToken, timeStr, dateStr, timestamp.toISOString()]);
...
function handleGetRollCallStatus(payload) {
  try {
    ...
    return sendResponse(true, 'OK', { active: active, rollCall: rollCall });
  } catch (error) {
...
// Danh sách đã xác nhận lần 2 của đợt roll-call MỚI NHẤT — dùng cho trang
// Admin so khớp với danh sách điểm danh gốc (ai chưa xác nhận = nghi ngờ rời
// sớm). Dùng doPost (cần biết meetingID, không phải thông tin mật nhưng để
// nhất quán với getAttendanceList vốn cũng POST được từ Admin qua callGAS).
function handleGetRollCallList(payload) {
  try {
    const meetingID = String(payload.meetingID || '').trim();
    if (!meetingID) return sendResponse(false, 'Thiếu meetingID', { list: [] });

    const rcSheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(rollCallSheetName(meetingID));
    if (!rcSheet) return sendResponse(true, 'Chưa có ai xác nhận lần 2', { list: [], token: null });

    const row = readMeetingConfigRow(meetingID);
    const currentToken = row && row.config && row.config.rollCall ? row.config.rollCall.token : null;

    const data = rcSheet.getDataRange().getValues();
    const list = [];
    for (let i = 1; i < data.length; i++) {
      if (!data[i][COL_RC.HOTEN - 1]) continue;
      // Chỉ lấy đúng đợt roll-call gần nhất (nếu admin đã kích hoạt nhiều lần)
      if (currentToken && data[i][COL_RC.TOKEN - 1] !== currentToken) continue;
      list.push({
        fullName: data[i][COL_RC.HOTEN - 1],
        time: data[i][COL_RC.THOIGIAN - 1],
        date: data[i][COL_RC.NGAY - 1]
      });
    }

    return sendResponse(true, 'OK', { list: list, token: currentToken });
  } catch (error) {
...
function fixMissingSTT() {
  ...
  sheets.forEach(sheet => {
    const name = sheet.getName();
    if (reserved.indexOf(name) !== -1) return;
    if (name.indexOf('rollcall_') === 0) return; // schema cột khác (COL_RC), không áp dụng STT kiểu điểm danh lần 1
...
function doGet(e) {
  ...
  if (action === 'getPositionSuggestions') {
    ...
    result = sendResponse(false, 'doGet chỉ hỗ trợ getPositionSuggestions / getDepartmentSuggestions / resolveMeetingCode / getMeetingConfigByID / getAttendanceList / getRollCallStatus / getRollCallList (JSONP). Các action khác dùng doPost().');
  }
